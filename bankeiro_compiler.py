...
class bankeiro(system_viewer):
    ...
    def get_info(self, text):
        ...
        self.allocation = [list(map(int, process.split()))
                           for process in info[1:(len(info)//2)+1:]]
        self.max_need = [tuple(map(int, process.split()))
    ...
    def get_available(self):
        ...
        for resource in range(len(self.max_resources)):
            av = self.max_resources[resource] - allocated[resource]
            available[resource] = av
    ...
    def get_remaining_need(self, number_process):
        ...
        for resource in range(len(self.max_resources)):
            rm = max_need[resource] - allocation[resource]
            remaining_need[resource] = rm
    ...
    def find_adequate_process(self):
        ...
        for index in range(len(self.allocation)):
            for resource in range(len(self.max_resources)):
                still_need = self.get_remaining_need(index)
                if still_need[resource] > available[resource]:
                    break
            else:
                return index
    ...
    def run_banker_s_algorithm(self):
        ...
        if process > -1:
            ...
            print('DeadLock Found on banker\'s algorithm!!!')
            return False

    def show_progress(self, stdsrc, process):
        stdsrc.clear()
        show_processes = ['|{:^10} {:^9}|'.format(str(self.allocation[index]),
                                                  str(self.max_need[index]))
                          for index in range(len(self.allocation))]

        self.show_list(stdsrc, show_processes,
                       axis_y=3, axis_x=3,
                       commands=True, pointer=process, title='Processes')
