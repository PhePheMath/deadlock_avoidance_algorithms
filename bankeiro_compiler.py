...
class system_viewer():
    ...
    def show_list(self, stdsrc, iterable, axis_y=1, axis_x=1, column_size=25, commands=False, pointer=-1, title='No Title'):
        title_config = '|{:^'+str((len(iterable)//column_size)*len(iterable[0])+len(iterable[0])-2)+'}|'
...
class bankeiro(system_viewer):
    ...
    def get_info(self, text):
        ...
        self.allocation = [list(map(int, process.split())) for process in info[1:(len(info)//2)+1:]]
        self.max_need = [tuple(map(int, process.split())) for process in info[(len(info)//2)+1::]]
    ...
    def get_available(self):
        ...
        for resource in range(len(self.max_resources)):
            available[resource] = self.max_resources[resource] - allocated[resource]
    ...
    def get_remaining_need(self, number_process):
        ...
        allocation = self.allocation[number_process]
        max_need = self.max_need[number_process]

        for resource in range(len(self.max_resources)):
            remaining_need[resource] = max_need[resource] - allocation[resource]
    ...
    def find_adequate_process(self):
        ...
        for index in range(len(self.allocation)):
            for resource in range(len(self.max_resources)):
                if self.get_remaining_need(index)[resource] > available[resource]:
                    break
            else:
                return index
    ...
    def show_progress(self, stdsrc, process):
        stdsrc.clear()
        show_processes = ['|{:^15} {:^15}|'.format(str(self.allocation[index]), str(self.get_remaining_need(index))) for index in range(len(self.allocation))]
        available = self.get_available()

        self.show_list(stdsrc, show_processes, 3, 45, commands=True, pointer=process, title='{:^15} {:^15}'.format('Have', 'Need'))
        stdsrc.addstr(3, 3, 'Máximo de recursos: {:^20}'.format(str(self.max_resources)))
        ...
        stdsrc.addstr(6, 3, title_config.format(str(self.get_remaining_need(process)), str(available)))
